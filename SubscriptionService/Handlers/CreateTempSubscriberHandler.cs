...
public class CreateTempSubscriberHandler : IRequestHandler<CreateTempSubscriberCommand, TempSubscriber>
{
    private readonly CoPartnerDbContext _dbContext;
    private readonly ISubscriberBusinessProcessor _subscriberBusineessProcessor;
    public CreateTempSubscriberHandler(CoPartnerDbContext dbContext, ISubscriberBusinessProcessor subscriberBusineessProcessor)
    {
        _dbContext = dbContext;
        _subscriberBusineessProcessor= subscriberBusineessProcessor;
    }

    public async Task<TempSubscriber> Handle(CreateTempSubscriberCommand request, CancellationToken cancellationToken)
    {
        var entity = request.TempSubscriber;
        ...
        using (var transaction = await _dbContext.Database.BeginTransactionAsync(cancellationToken))
        {
            try
            {
                ...
                await _dbContext.TempSubscribers.AddAsync(entity, cancellationToken);
                ...
                request.TempSubscriber.Id = entity.Id;

                await transaction.CommitAsync(cancellationToken);

                return request.TempSubscriber;
            }
            catch (Exception)
            {
